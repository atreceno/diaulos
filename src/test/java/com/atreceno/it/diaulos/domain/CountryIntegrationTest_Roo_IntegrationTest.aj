...
import com.atreceno.it.diaulos.domain.CountryIntegrationTest;
...
privileged aspect CountryIntegrationTest_Roo_IntegrationTest {
    ...
    public void CountryIntegrationTest.testFindAllCountrys() {
        Assert.assertNotNull("Data on demand for 'Country' failed to initialize correctly", dod.getRandomCountry());
        long count = Country.countCountrys();
        Assert.assertTrue("Too expensive to perform a find all test for 'Country', as there are " + count + " entries; set the findAllMaximum to exceed this value or set findAll=false on the integration test annotation to disable the test", count < 300);
